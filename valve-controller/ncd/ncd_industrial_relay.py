class Relay_Controller:
    ...
    def start_relay_timer(self, timer, hours, minutes, seconds, relay):
        command = self.wrap_in_api(
            [254, 50, 49 + timer, hours, minutes, seconds, relay - 1]
        )
        return self.process_control_command_return(self.send_command(command, 4))
    ...
    def read_single_ad8(self, channel):
        ...
        return self.translate_ad(
            self.process_read_command_return(self.send_command(command, 4)), 8
        )

    def read_all_ad8(self):
        command = self.wrap_in_api([254, 166])
        return self.translate_ad(
            self.process_read_command_return(self.send_command(command, 11)), 8
        )

    def read_single_ad10(self, channel):
        command = self.wrap_in_api([254, 157 + channel])
        return self.translate_ad(
            self.process_read_command_return(self.send_command(command, 5)), 10
        )

    def read_all_ad10(self):
        command = self.wrap_in_api([254, 167])
        return self.translate_ad(
            self.process_read_command_return(self.send_command(command, 19)), 10
        )
    ...
    def reactor_read_timers(self):
        ...
        result, seconds = self.process_control_command_return(
            self.send_command(command, 16)
        )
        command = self.wrap_in_api([254, 55, 3, 120, 16])
        result, minutes = self.process_control_command_return(
            self.send_command(command, 16)
        )
        command = self.wrap_in_api([254, 55, 3, 104, 16])
        result, hours = self.process_control_command_return(
            self.send_command(command, 16)
        )
        return (
            self.hex_to_decimal(seconds),
            self.hex_to_decimal(minutes),
            self.hex_to_decimal(hours),
        )

    def reactor_send_event(self, event):
        command = self.wrap_in_api([233, 108, int(event)])
        result, data = self.process_control_command_return(
    ...
    def lantronix_read_amps(self):
        command = self.wrap_in_api(
            [188, 50, 10, 84, 146, 106, 1, 1, 6, 0, 0, 4, 85, 19]
        )
        return self.process_control_command_return(self.send_command(command, 32))
